...
| command | what | needs |
|---|---|---|
| `npm run test:offline` | `tests/offline.sh`: semgrep against a fake Jev (`tests/fake-jev.mjs`). Expression, output shapes, `-A/-B/-C`, `--level`/`-t`/`-T`, `-p`, `--color`, `--chunk`, `-j`, `-q` (including the stop at the first match), `-Q`, `SEMGREP_URL` and the auth header, `--help`; `tests/literals.mjs` (required literals, #80) | node, curl. No key, no network, same result every run, ~15s |
