...
言語や形式を指定する意味、変更時期を指定する意味は、そういうファイルの中でしか当たりません。`-r` と
`git semgrep` ではそのファイルだけを探し、残りは読まず、送りもしません。絞り込みは stderr に出るので、
誤りに気づけます。
...
semgrep: scope: changed since 2026-09-25 00:00 (git commits; the mtime for files git does not have committed) (from "昨日")
semgrep: scope: 3 of 120 files
```

- **言語・形式**: 「Python で」「Python のコード」「Go 言語のコード」「.py ファイルで」「YAML ファイル」「in Python」
  「Rust or Kotlin implementations」。「Python のような書き方」「Go に移植する」「Python ではない」は、ファイルに
  ついて何も言っていないので絞りません。SQL・HTML・CSS・JSON・XML は他の言語のコードの中にもよくあるので、
  「SQL ファイル」のようにファイルを指したときだけ絞ります (「SQL のクエリ」では絞らない)。
- **置き場所**: テストコード (「テストコードで」「in the tests」)、マイグレーション、README、CHANGELOG、文書
  (「ドキュメントに」「in the docs」。`*.md *.rst *.adoc *.txt`、`docs/`)、コード (文書以外すべて。辞書に無い言語も
  含む)、ログ (「ログファイルに」「in the logs」。`*.log`、`logs/`)。JS・Python・Go・Java・Ruby・Rust・PHP の
  パスの慣習で判定します。Rust は単体テストを同じファイルに書くので、テストコードには `*.rs` をすべて含めます。
  1 つの意味に複数の置き場所があれば、どれか (「README か CHANGELOG に」)。他のものと並べた置き場所
  (「テストとフィクスチャで」) では絞りません。「テストしている」「README を生成する」は、コードがすることで
  あって置き場所ではないので絞りません。
- **変更時期**: 変更を表す動詞と並んだ日付や期間。「昨日変えた」「先週追加した」「9月20日以降に修正した」
  「ここ 3 日でコミットした」「changed yesterday」。リポジトリの中のコミット済みのファイルは、その時刻以降の
  コミットがあるものに絞ります (コミッター日時。checkout は更新時刻をすべて今にしてしまい、コミットは記録する
  編集より後に来るため)。未コミットのファイルとリポジトリの外のファイルは、その時刻以降に更新したものに
  絞ります。上限は設けません (昨日の変更を今日コミットすることがあるため)。行の内容が話している日付 (「9月20日のリリース」
  「昨日のログ」)、「以前」「まで」、あいまいな言葉 (「最近」) では絞りません。
- **作者と状態 (git)**: 「自分が書いた」「Alice さんが書いた」「code I wrote」「written by Alice」(git の
  `--author`、mailmap 適用。その人のコミットが触れたファイルすべて。自分なら未コミットのファイルも含む。
  コミットが 1 つも無い名前では絞らない)、「未コミットの」(作業ツリーとインデックスの HEAD との差分、未追跡の
  ファイル)、「ステージした」、「未追跡の」、「このブランチで」(`origin/HEAD`・`main`・`master` から分かれた点から
  作業ツリーまで)、「未プッシュの」(`@{upstream}..HEAD`、無ければどのリモートブランチにも無いコミット)。
  リポジトリの外では絞りません。作者が書いた後で名前が変わったファイルは漏れます。
- **項ごとに効く。** `-e A -e B` は A の絞り込みで除いたファイルでも B を探します。同じ AND 項の中では
  絞り込みが重なります。否定した意味 (`-v`、`!`) は絞りません。意味の文面はそのまま送ります。
- コマンドラインで指定したファイルと stdin は、`--include` と同じく絞りません。`--no-scope` で止められます。
