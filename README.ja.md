...
Jev は Vercel AI Gateway、OpenRouter、Cloudflare Workers AI からも使えます。semgrep は
キーの接頭辞で経路を決めるので、キーを置くだけで済みます。

| キー | 経路 | モデル | 変数 |
|---|---|---|---|
| `vck_…` | Vercel AI Gateway | `typesafe-ai/jev` | `AI_GATEWAY_API_KEY` か `SEMGREP_API_KEY` |
| `sk-or-…` | OpenRouter | `jev-1.13` | `OPENROUTER_API_KEY` か `SEMGREP_API_KEY` |
| `<32 桁 16 進のアカウント ID>:cfut_…`（`cfat_` も可） | Cloudflare Workers AI | `typesafe/jev` | `SEMGREP_API_KEY` |
| それ以外 | TypeSafe に直接 | `jev-latest` | `TYPESAFE_API_KEY` か `SEMGREP_API_KEY` |

```sh
export OPENROUTER_API_KEY=sk-or-...
export SEMGREP_API_KEY="<account-id>:<cfut_token>"   # Cloudflare はアカウント ID とトークンを : でつなぐ
```

キーは `SEMGREP_API_KEY`、`TYPESAFE_API_KEY`、`AI_GATEWAY_API_KEY`、`OPENROUTER_API_KEY` の順に探します。
`CLOUDFLARE_API_TOKEN` は他のツール用に設定されていることが多いので、Cloudflare のトークンはこの形で明示したときだけ使います。
`SEMGREP_URL` と `SEMGREP_MODEL` で送信先とモデル名を上書きできます。`SEMGREP_URL` が既知のプロバイダのホストを指すときは、
そのプロバイダのキーだけを送ります。集計行には API が返す費用（OpenRouter のように `usage.cost` があれば）を、無ければ
`SEMGREP_PRICE_PER_M`（既定は 100 万入力トークンあたり 0.042 ドル）での推定値を `~` 付きで表示します。
